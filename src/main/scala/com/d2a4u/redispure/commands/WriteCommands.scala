...
case class Set[F[_], T](
  key: String,
  value: T,
  px: Option[Long] = None,
  nx: Boolean = false,
  xx: Boolean = false
)(implicit encoder: Encoder[T], client: RedisClient, F: Effect[F]) extends RESPCmd[RESimpleString.type] with FRun[F, RESimpleString.type] {
  override def run(): F[Either[Throwable, RESimpleString.type]] = {
    val ioAction: F[Either[SerializationException, Either[Throwable, RESimpleString.type]]] =
      encoder.encode(this.value).traverse { encodedValue =>
        val reArray: REArray =
          RENeArray({
            val arr: Array[RESP] = Array(
              REString("SET"),
              REString(this.key),
              REString(encodedValue)
            )
...
object Set {
  def apply[F[_]](
    key: String,
    value: String
  )(implicit client: RedisClient, F: Effect[F]): Set[F, String] = Set(key, value, None, false, false)
}

case class Append[F[_], T](
  key: String,
  value: T
)(implicit encoder: Encoder[T], client: RedisClient, F: Effect[F]) extends RESPCmd[REInt] with FRun[F, REInt] {
