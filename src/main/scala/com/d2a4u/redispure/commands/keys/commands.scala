...
case class Del[F[_]](keys: String*)(implicit client: RedisClient[F], F: Effect[F]) extends BasicRESPCmd[F, REInt] {
  override val cmd: REArray = REArray("DEL") |+| RENeArray(keys.toArray.map(REString.apply))
}

case class Exists[F[_]](keys: String*)(implicit client: RedisClient[F], F: Effect[F]) extends BasicRESPCmd[F, REInt] {
