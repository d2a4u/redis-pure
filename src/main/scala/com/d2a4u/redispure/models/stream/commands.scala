...
case class XKeyValue(
  key: String,
  value: String
)

case class XaddCmd[F[_]](
  id: String,
  key: String,
  entries: XKeyValue*
)(implicit client: RedisClient, F: Effect[F]) extends RESPCmd[REBulkString] with FRun[F, REBulkString] {
...
object XaddCmd {
  def apply[F[_]](
    key: String,
    values: XKeyValue*
  )(implicit client: RedisClient, F: Effect[F]): XaddCmd[F] = XaddCmd("*", key, values:_*)
}

case class XlenCmd[F[_]](
  key: String
)(implicit client: RedisClient, F: Effect[F]) extends RESPCmd[REInt] with FRun[F, REInt] {
