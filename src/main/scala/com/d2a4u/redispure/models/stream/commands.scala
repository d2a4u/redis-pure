...
import com.d2a4u.redispure.Error

case class XKeyValue(key: String, value: String)

case class XaddCmd[F[_]](id: String, key: String, entries: XKeyValue*)(implicit client: RedisClient[F], F: Effect[F])
    extends RESPCmd[REBulkString]
    with FRun[F, REBulkString] {
  override def run(): F[Either[Error, REBulkString]] = {
...
object XaddCmd {
  def apply[F[_]](key: String, values: XKeyValue*)(implicit client: RedisClient[F], F: Effect[F]): XaddCmd[F] =
    XaddCmd("*", key, values: _*)
}

case class XlenCmd[F[_]](key: String)(implicit client: RedisClient[F], F: Effect[F])
    extends RESPCmd[REInt]
    with FRun[F, REInt] {
  override def run(): F[Either[Error, REInt]] = {
